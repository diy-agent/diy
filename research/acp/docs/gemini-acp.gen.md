...
date: 2026-03-24
---
<!-- This file was automatically generated by diy. DO NOT EDIT. -->
...
## 8. diy-core AcpClientAdapter 设计方案

### 8.1 设计目标

基于 Gemini CLI 的 ACP 实现模式，在 diy-core 中设计通用的 `AcpClientAdapter`，实现对所有支持 ACP 协议的外部 Agent 的统一接入。

### 8.2 核心接口设计

```typescript
// packages/diy-core/src/acp-adapter.ts
...
export class AcpClientAdapter implements CodingAgent {
  ...
  async initialize(): Promise<void> {
    ...
    const client: acp.Client = {
      async requestPermission(params) {
        // TODO: Map to diy TUI prompt
        return { outcome: { optionId: 'proceed_once' } };
      },
      
      async fsRead(params) {
        // TODO: Map to diy file system
...
2. **Phase 2: 回调集成**
   - 实现 requestPermission 映射到 diy TUI
...
基于 ACP 的通用适配器设计，diy-core 可以实现对所有支持 ACP 协议的外部 Agent 的统一接入，包括未来的 Claude CLI、其他 AI 编程工具以及自定义 MCP 服务器。
