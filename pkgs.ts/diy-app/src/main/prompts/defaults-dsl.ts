// src/main/prompts/defaults-dsl.ts
// 🎯 内置提示词模版（DSL 版）—— 新引擎（@diy/template）的唯一真相源
//
// 与 legacy 版（defaults.ts + 代码拼装）的关系：
//   本文件是 M4 的迁移目标：**节标签内联在模版里、节顺序与分隔写在 system.md 里**，
//   代码不再拼 `<tag>`、不再 join("\n\n")、不再 trim。验收标准是"模型看到的东西不变"
//   （见 tests/core/template-dsl-golden.test.ts 与 tests/fixtures/system.golden.txt）。
//
// 写法约定（配合引擎语义）：
//   · 控制标记不占空间：独占一行的 <template …> / 注释，连该行缩进与换行都不产出
//   · 节间分隔由**节模版自己的末尾换行** + system.md 里的空行共同表达
...
export const PROMPT_DEFAULTS_DSL: Record<string, string> = {
    // ── 装配入口（锁定）：顺序即节顺序，空行即节间分隔 ──────────────
    'system.md': `---
title: 系统上下文装配
desc: 节顺序与节间分隔（装配入口，锁定不可覆盖）。
version: 1
overridable: { value: false, tip: "装配入口是结构契约：改了会改变模型看到的节顺序与分隔。仅可查看" }
tag: ""
---
...
<template :include="./500-skills.md" :if="skills" list="skills"/>
