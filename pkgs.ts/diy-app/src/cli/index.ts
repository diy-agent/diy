...
function launchApp(cfg: RuntimeConfig): ChildProcess {
  ...
  // _DIY_MIRROR_DISPLAY=1: 内部变量，窗口定位到副屏（iPad Sidecar），避免遮挡主屏
  // 由入口脚本注入（diy.sh / electron-dev.mts），CLI 仅透传 process.env
  ...
  // app.commandLine.appendSwitch 生效，此处跟在 app 路径后传 argv 无效，故不传。
  //
