...
### 2.0.2 `knownTags`：让指定标签也能带控制属性（**默认关闭，待定**）

`<description :if="…">` / `<skill :for="…">` 这类写法需要"可解析标签白名单"（引擎选项 `knownTags`，默认空 = 只有 `template`/`raw`）。
实测碰撞（剔代码围栏后，1703KB 语料）：

| 候选标签 | 剔围栏碰撞 | 结论 |
| --- | ---: | --- |
| `<description>` / `<example>` | **0 / 0** | 安全（含围栏内也只是文档示例） |
| `<documentation>` / `<skill>` | 0 / 0 | 安全（目前尚不存在这两个标签） |
| `<diy>` / `<rules>` / `<skills>` | 17 / 10 / 8 | 撞的是我们自己的文档（是"值"不进 parser）；但**模版里想"讲"这些标签就必须进围栏** |
| `<guard>` / `<project_context>` / `<task>` | 5 / 4 / 4 | 同上 |

**长期代价**：① 每新增一个可解析标签都要登记，漏登记 → 静默变文本（靠 lint 兜）；
② 两种写法并存（`<template :if>` 包裹 vs `<xxx :if>`），需要在模版规范里指定推荐用法。
**建议**：默认关；等 M4 或后续真的出现 `<description :if>` 这类需求时，加一个小白名单（`description`/`skill` 等 0 碰撞的名字）。
...
5. **独占一行的控制标记整行不输出**（standalone 规则）——**建议，待确认，尚未实现**；
   不开也能工作（分隔符由节模版自带，§8 已拍板项 (b)），开了能让布局更好读并消除"空节残留空行"。
