...
### 2.0.2 判据 C：**带控制属性的标签才是节点**（不需要白名单，也不需要配对校验）

`<description :if="…">` / `<skill :for="…">` 这类写法**不需要 `knownTags` 白名单**，判据是结构性的：
...
2. **当且仅当标签头里带控制属性**（`:if` / `:unless` / `:for`）时，它成为**容器节点**：
   必须找到配对的 `</name>`（找不到 → **报错**，不静默），作用是"按条件/循环渲染内部，并把自身的
   开/闭合标签**原样**输出"。⇒ `<skill :for="s of skills">{{s.name}}</skill>` → `<skill>a</skill><skill>b</skill>`。
3. **逐字节重发**：只按**字符区间**剥掉控制属性，其余字符（引号、空白、无值属性）原样保留
   （`<pi path='x'  flag :if="on">` → `<pi path='x'  flag>`），**绝不重新格式化**。

`<template>` 是同判据的特例（`:if`/`:unless`/`:for`），区别只在于**不输出自身标签**；
