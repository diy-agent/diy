...
\{{                            逃生舱 1：输出字面量 {{
\<                             逃生舱 2：输出字面量 <（正文里的 a<b、<tag>、</tag> 都靠它）
<raw>…</raw>                   逃生舱 3：整段原样输出
...
4. **模版 body 的末尾换行 / trim**：现有注册表对 body 做 `trim() + '\n'`，新引擎**不 trim**。
   → 注册表是否停止补 `\n`（模版源逐字节进引擎）？用户覆盖文件的末尾换行是否照原样参与渲染？
   这是 byte-diff 能否达成的关键一条（当前等价测试用固定形态绕过了它）。
5. **节间分隔与空节**：`join("\n\n")` 有两种表达（布局行间空行 / 片段前置分隔符）。
   实测：**被 `:if` 跳过的节会把它前后的空行留下**（多一个空行）。
   → 接受（可预测）／改用"节自带分隔符"／新增"独立控制行不输出"规则（Mustache 的 standalone 语义，属隐式魔法）。
