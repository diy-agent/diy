...
const GLOBAL_CACHE_DIR = path.join(os.homedir(), ".diy");
...
export async function syncDependencies(): Promise<void> {
  ...
  const projectConfigPath = path.resolve(process.cwd(), "diy.yaml");
  ...
  // 输出 dependency mirror 索引到 .diy/ref，使索引和镜像目录生命周期保持一致
