...
import { PiCodingAgent } from "@diy-agent/diy-core";
...
async function main() {
  ...
  await using agent = new PiCodingAgent({
    instructions:
      "You are a helpful coding assistant. You have access to file read, bash, edit, and write tools.",
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "tool_call_response":
        console.log(
          `[Tool Result: ${event.value.name} - ${event.value.success ? "OK" : "Failed"}]`,
        );
        break;
