# diy-core - Core Abstractions for Coding Agent Orchestration

## Overview

`diy-core` provides shared runtime abstractions and interfaces for orchestrating multiple coding agents through a unified CLI workflow.
...
Implement a generic `AcpClientAdapter` that allows **diy** to spawn and communicate with any external ACP-compatible coding agent as a subprocess. ACP (Agent Client Protocol) is an open standard that enables communication between code editors and AI coding agents.
...
4. Allow diy-cli/diy-tui to handle permissions and file operations via existing UI

### Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                     diy-cli / diy-tui (UI Layer)                 │
...
- [ ] **Permission request handling** - Map ACP `requestPermission` to diy permission callback
- [ ] **File system operations (fsRead/fsWrite)** - Map to diy file system
- [ ] **MCP server configuration** - Pass through from client options
- [ ] **Cancel support** - `connection.cancel()`
- [ ] **Error handling** - Map ACP error codes to diy errors
...
2. **Event mapping**: ACP pushes incremental updates via `sessionUpdate` notification. Map these to diy's internal `AgentStreamEvent` type.

3. **Permission flow**: When ACP agent requests permission, forward the request to diy's UI layer (diy-tui), user makes the choice, send the choice back to ACP agent.

4. **File system integration**: If the ACP agent requests file operations via `fsRead`/`fsWrite`, let diy handle them using the local file system. This gives consistent behavior across all agents.
...
- [ ] Full permission callback integration with diy TUI
